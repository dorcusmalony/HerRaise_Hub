...
export default function Register(){ // renamed from Signup back to Register
	...
	const submitToServer = async (role) => {
		...
		try {
			const res = await fetch(`${API}/register`, { // changed back from /signup to /register
				method: 'POST',
			...
			if (role === 'mentor') {
				...
				const fakeResponse = { success: true, message: 'Registered as mentee (simulated)', user: { name: payload.name, email: payload.email, role: 'mentee' } } // restored message
				setResult(fakeResponse)
			}
		} finally {
			setSubmitting(false)
	...
	return (
		<div className={`mx-auto ${styles.container}`}>
			<h3>Create account</h3> {/* restored original header */}
